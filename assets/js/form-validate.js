	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#Bake Field Cakes-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#Bake Field Cakes-contact').find('input,textarea').attr({value:''});
				jQuery('#Bake Field Cakes-contact').css({opacity:1});
				jQuery('#Bake Field Cakes-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#Bake Field Cakes-contact').animate({opacity:0.3});
		jQuery('#Bake Field Cakes-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#Bake Field Cakes-contact').find('input,textarea').css('border','none');
